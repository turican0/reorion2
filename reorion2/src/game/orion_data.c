...
// Data declarations
//
// wave 180: every data symbol of the original data object (C 0x178000..
// 0x1D5CD0) now lives in `dseg` (orion_dseg.c, generated by
// tools/compare/gen_dseg.py) with the original layout and the bytes of the
// EXE; the names are macros (orion_dseg.h). What is left here: code-segment
// data, port pointer variables (REAL_POINTERS in gen_dseg.py), port structs
// and deliberate port changes. Many older comments below describe
// declarations that moved to dseg - read them as history.
